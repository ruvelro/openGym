...
**This fork publishes no signed APK yet** — build your own below. The *original project* does
publish one, signed by its author and built by its GitLab CI: on
**[opengym.duarte-santos.ch](https://opengym.duarte-santos.ch)** and in
[its GitLab package registry](https://gitlab.com/DuarteSantos8/opengym/-/packages) with a
`.sha256` beside it. Same app, but its in-app links point at the original repository, not this
one. Android asks you to allow installs from the browser the first time — that's standard for
